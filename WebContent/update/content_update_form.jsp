...
<% 
	...
	DTOContent oldContent = daoContent.readContent(contentCode);
	String cate = oldContent.getContent_cate();
%>
<script>
	$(function(){
		var sendCate = <%=cate%>;
...
<form id="contentWriteForm" action="<%= request.getContextPath()%>/update/content_update_pro.jsp" method ="post">
		<input type="hidden" name="content_code" value="<%=oldContent.getContent_code()%>">
		<input type="hidden" name="user_id" value="<%=S_ID%>">
		<input type="hidden" name="content_desc" >
      		<input type="text" name="content_title" class="form-control" id="title" placeholder="제목을 입력하세요!" value="<%= oldContent.getContent_title() %>">
		...
		<div id="editor-container" >	
			<%=oldContent.getContent_desc() %>
		</div>
			<button type="button"  class="writeBtn btn btn-primary " id="writeBtn">게시글 수정</button>
	</form>
